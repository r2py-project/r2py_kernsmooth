# Negative test cases for r2py_kernsmooth.dpik
#
...
#
# Known behavioral divergences (R vs Python) are explicitly documented with
...
# ---------------------------------------------------------------------------
# Helper: call R dpik and capture errors
# ---------------------------------------------------------------------------

def _r_dpik_catch(x_py, **kwargs):
    """
    Call R dpik and return (raised: bool, message: str).
...
def test_dpik_level_negative_behavioral_divergence():
    ...
    empty vector.  The Python port uses elif chains so 'psi4hat' is never
    assigned, causing an UnboundLocalError.  The test documents the divergence
    and emits a warning.
    """
    x = np.arange(1.0, 11.0)
    r_raised, r_msg = _r_dpik_catch(x, level=-1)
...
def test_dpik_invalid_kernel_raises():
    """kernel='gaussian' (unrecognised) must raise ValueError in Python (matches R)."""
    x = np.arange(1.0, 11.0)
    r_raised, r_msg = _r_dpik_catch(x, kernel="gaussian")

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, kernel="gaussian")

    _check_both_raise(r_raised, r_msg, exc_info, label="kernel='gaussian'")


def test_dpik_invalid_kernel_empty_string_raises():
    """kernel='' must raise ValueError in Python (matches R)."""
    x = np.arange(1.0, 11.0)
    r_raised, r_msg = _r_dpik_catch(x, kernel="")

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, kernel="")

    _check_both_raise(r_raised, r_msg, exc_info, label="kernel=''")


def test_dpik_invalid_kernel_ambiguous_prefix_raises():
    """kernel='b' is ambiguous ('box' vs 'biweight') and must raise ValueError."""
    x = np.arange(1.0, 11.0)
    # R uses match.arg which also raises for ambiguous prefixes.
    r_raised, r_msg = _r_dpik_catch(x, kernel="b")

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, kernel="b")

    _check_both_raise(r_raised, r_msg, exc_info, label="kernel='b' ambiguous")
...
def test_dpik_invalid_scalest_unknown_raises():
    """scalest='unknown' must raise ValueError in Python (matches R)."""
    x = np.arange(1.0, 6.0)
    r_raised, r_msg = _r_dpik_catch(x, scalest="unknown")

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, scalest="unknown")

    _check_both_raise(r_raised, r_msg, exc_info, label="scalest='unknown'")


def test_dpik_invalid_scalest_empty_string_raises():
    """scalest='' (empty string) must raise ValueError in Python (matches R)."""
    x = np.arange(1.0, 6.0)
    r_raised, r_msg = _r_dpik_catch(x, scalest="")

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, scalest="")
...
def test_dpik_n1_raises():
    """n=1 observation: R raises (missing value in comparison); Python also raises."""
    x = np.array([5.0])
    r_raised, r_msg = _r_dpik_catch(x)
...
def test_dpik_inf_in_data_raises():
    """Inf in x: R raises ('to' must be a finite number); Python should also raise."""
...
def test_dpik_neg_inf_in_data_raises():
...
def test_dpik_nan_in_data_raises():
...
def test_dpik_empty_array_raises():
...
def test_dpik_range_x_equal_endpoints_raises():
    """range_x with a==b creates a zero-width grid; both R and Python should raise."""
    x = np.arange(1.0, 6.0)
    r_raised, r_msg = _r_dpik_catch(x, range_x=(2.0, 2.0))

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, range_x=(2.0, 2.0))

    _check_both_raise(r_raised, r_msg, exc_info, label="range_x a==b")


# ---------------------------------------------------------------------------
# level=6 for every kernel (level check happens before kernel dispatch)
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("kernel", ["normal", "box", "epanech", "biweight", "triweight"])
def test_dpik_level_6_raises_for_all_kernels(kernel):
    """level=6 must raise regardless of kernel choice."""
    x = np.arange(1.0, 11.0)
    r_raised, r_msg = _r_dpik_catch(x, level=6, kernel=kernel)

    with pytest.raises(Exception) as exc_info:
        r2py_kernsmooth.dpik(x, level=6, kernel=kernel)

    _check_both_raise(r_raised, r_msg, exc_info, label=f"level=6 kernel={kernel}")
